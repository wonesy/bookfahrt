...
func (uq *UserQuery) QueryClubs() *ClubQuery {
...
func (uq *UserQuery) QueryInvitations() *InvitationQuery {
	query := &InvitationQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) WithClubs(opts ...func(*ClubQuery)) *UserQuery {
	query := &ClubQuery{config: uq.config}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withClubs; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(user.ClubsPrimaryKey[0], fks...))
			},
	...
	if query := uq.withInvitations; query != nil {
		...
		for _, n := range neighbors {
			fk := n.invitation_sponsor
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "invitation_sponsor" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "invitation_sponsor" returned %v for node %v`, *fk, n.ID)
			}
